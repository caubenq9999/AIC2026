    ...
    def __init__(
        ...
        if shard_filename:
            shard_paths = sorted(
                (
                    path
                    for path in self.vectors_dir.glob(f"L*/{shard_filename}")
        ...
        if not shard_paths:
            expected = f"L*/{shard_filename}" if shard_filename else "L*.npy"
            raise FileNotFoundError(f"Không có shard {expected} trong {self.vectors_dir}")

        offset = 0
        for shard_path in shard_paths:
            collection = (
                shard_path.parent.name.upper()
                if shard_filename else shard_path.stem.upper()
            )
